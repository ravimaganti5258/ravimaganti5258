...
const DropdownItem = ({
  ...
  return (
    <ItemContainer
      ...
      style={{
        // backgroundColor:
        //   selected && !nested ? Colors.darkSecondaryTxt : Colors.white,
      ...
      <Text
        ...
        size={textSizes.h11}
        fontFamily={!nested ? fontFamily.semiBold : fontFamily.regular}>
        {item?.value}
      </Text>
    </ItemContainer>
  );
...
export const NestedDropDown = ({
  ...
  const [showNestDropDown, setnestedDropdown] = useState(false);
  const [itemClicked, setClickItem] = React.useState({});

  useEffect(() => {
    setShowDropdownItem(showDropDown)
  }, [showDropDown])
  ...
  const handleCategoryPress = React.useCallback((item) => {
    handleSelectionCategory(item);
  }, []);
      ...
      <View key={`ID-${index}`}>
        <DropdownItem
          ...
          onPress={() => {
            ...
            setClickItem(item);
            // item?.id == '1' && handleSelection(item);
            // handleItemPress(item);
          }}
        />
        {showNestDropDown && selectedCategory.id == item.id && (
          <View >
            {subList.map((item, index) => {
              if (item.mainValue == selectedCategory.value) {
                return (
                  <DropdownItem
                    key={`ID-${index}`}
                    item={item}
