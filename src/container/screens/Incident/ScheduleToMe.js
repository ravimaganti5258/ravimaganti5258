...
import { pricedetails } from '../../../assets/jsonData';
import MultiButton from '../../../components/MultiButton';

const { width, height } = Dimensions.get('screen');

const ScheduleToMe = ({ visibility, handleModalVisibility, dataItem }) => {
  const todayDate = new Date();
  const currentDate = dateFormat(todayDate, 'YYYY-MM-DD');
  const [date, setDate] = useState(currentDate);
  const [onGoData,setOnGoData] = useState([])
  ...
  const priceDetailsData = priceData.filter(
    (obj) => obj.WoJobId == jobInfo?.WoJobId,
  );
  const calenderTheme = {
    ...
    try {
      ...
      const handleCallback = {
        ...
        error: (error) => {
          ...
          console.log('err in schedule', error);
          dispatch({ type: SET_LOADER_FALSE });
        },
      };
  ...
  const onPressGo = () => {
    try {
      ...
      const handleCallback = {
        success: (data) => {
          setAdd(!add);
          // FlashMessageComponent('success', 'Added sucessfully');
          setTimedata(data);
          setFlag(true);
          dispatch({ type: SET_LOADER_FALSE });
        },
        error: (err) => {
          console.log('err in schedule', err);
          dispatch({ type: SET_LOADER_FALSE });
        },
      };
      const header = Header(token);
          ...
          {flag === true ? (
            <View>
              ...
              {timedata.length > 0 ? (
                ...
                <Text style={{ marginTop: normalize(10) }}>
