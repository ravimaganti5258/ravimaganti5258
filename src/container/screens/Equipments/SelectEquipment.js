...
const SelectEquipment = ({ navigation }) => {
  ...
  const allEquipmentAttachment = useSelector(
    (state) => state?.EquipmentReducer?.data,
  );
    ...
    try {
      ...
      const handleCallback = {
        success: (data) => {
          ...
          //  setSelectEquipmentListData(FiltredData);
          //  setSelectEquipListData(FiltredData);
      ...
      {!showQR ? (
        <View style={{ backgroundColor: '#F9F9F9', flex: 1 }}>
          ...
          {/* <View
            ...
            <Text style={{color: 'white', paddingRight: normalize(15)}}>
              Job #{jobInfo?.WoJobId}
            </Text>
          </View>
