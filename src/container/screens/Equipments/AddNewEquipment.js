...
import {
  _storeLocalAddEquipmentObj,
  _updateLocalEquipment,
} from '../../../database/JobDetails/addNewEquipment.js';
...
const AddNewEquipment = ({navigation, route}) => {
  ...
  const toggleExistsEquipmentModal = () => {
    setShowEquipmentExistsModal(!showEquipmentExistsModal);
  };
  const handleEquipmentConfirm = () => {
    navigation.navigate('SelectEquipment');
    setShowEquipmentExistsModal(false);
  };
  useEffect(() => {
      ...
      WoEquipmentEntity: [
        {
          ...
          ContractTypeId: selectedContractTypeDropDown?.id
            ? selectedContractTypeDropDown?.id
            : type?.id,
          ContractType: selectedContractTypeDropDown?.value
            ? selectedContractTypeDropDown?.value
            : type?.value,
  ...
  const editEquipmentApi = () => {
    const testDate =
      contractStartDate != '' ? contractStartDate.split('-') : null;
  ...
  const validation = async () => {
    ...
    if (brandEmpty == true) {
      FlashMessageComponent(
        'warning',
        strings('SelectEquipment.Brand_must_be_selected'),
      );
    } else if (modelEmpty == true) {
      FlashMessageComponent(
        'warning',
        strings('SelectEquipment.Model_must_be_selected'),
      );
    }
    ...
    switch (caseId) {
      case 1:
        FlashMessageComponent(
          'warning',
          strings(
            'flashmessage.Manufacture_Date_and_Expiry_Date_cannot_be_same',
          ),
        );
        break;
      case 2:
        FlashMessageComponent(
          'warning',
          strings(
            'flashmessage.Expiry_Date_cannot_be_before_Manufacture_Date_and',
          ),
        );
        break;
    }
          ...
          <ScrollView
                ...
                {NestedUI == true && (
                  <View>
                    ...
                    <Dropdown
                      ...
                      handleSelection={(val) => {
                        setSelelctedSlaDropDown(val), setDrop3(false);
                      }}
                      zIndexVal={0}
                      align={'flex-start'}
                      placeholder={''}
                    ...
                    <Dropdown
                      ...
                      hasBorder={true}
                      label={
                        selectedContractTypeDropDown
                          ? Object.keys(type).length != 0
                            ? type?.label
                            : selectedContractTypeDropDown?.label
                          : type
                      }
            ...
            {showAddEquipment ? (
              <AddContractTypeModal
                ...
                setType={(val) => {
                  setType(val);
                  const data = [...contractTypeArray, val];
                  setContractTypeArry(data);
                }}
