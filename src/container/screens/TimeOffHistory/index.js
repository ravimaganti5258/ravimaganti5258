...
const TimeOffHistory = ({ navigation }) => {
  ...
  // };
  const renderItem = ({item, index}) => {
    const startTime = splitDateString(item?.StartTime);
    const endTime = splitDateString(item?.EndTime);
    let yearTwoDigStr = startTime[0].slice(2, 4);
    let yearTwoDigEnd = endTime[0].slice(2, 4);
    // var frmtDate = moment(item?.SendDate).format('Do MMM, h:mm A');
    if(item?.Days ==1){
    ...
    if (frmtStartDate != '' && frmtEndDate != '') {
      ...
      // onSelectTime(item?.StartTime, item?.EndTime)
    }
    }
    let totalHours = '' ;
        ...
        <WrapperComponent
          ...
          <TouchableOpacity
            ...
            <TimeOffCard
              ...
              date={`${startTime[2]} ${months[startTime[1] - 1]
                } ${startTime[0].slice(2, 4)} - ${endTime[2]} ${months[startTime[1] - 1]
                } ${endTime[0].slice(2, 4)}`}
            />
          </TouchableOpacity>
        </WrapperComponent>
  ...
  const reducer = (acc, cur) => {
    try {
      ...
      const appliedOn = splitDateString(cur?.StartTime);
      const item = acc.find(
        (x) => x.month === `${months[appliedOn[1] - 1]} ${appliedOn[0]} `,
      );
      if (item) {
        item.data.push(cur);
      } else {
        acc.push({
          month: `${months[appliedOn[1] - 1]} ${appliedOn[0]} `,
          data: [cur],
        });
      }
      return acc;
      ...
      <SafeAreaView style={styles.safeAreaContainer}>
        <View style={styles.sectionListContainer}>
          {timeOffHistory.length > 0 ? (
            ...
            //   <Text size={textSizes.h9}>{message}</Text>
            // </View>
          )}
          ...
          {/* {showDeleteModal ? (
            <ConfirmationModal
              title={'COnfirmation'}
              discription={'Are you sure want to Delete ?'}
              handleModalVisibility={toggleDeleteModal}
              visibility={showDeleteModal}
              handleConfirm={handleDeleteTimeOff}
            />
          ) : null} */}
        </View>
      </SafeAreaView>
