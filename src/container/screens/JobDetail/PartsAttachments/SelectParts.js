...
const SelectPartsCard = memo(
  ({item, index, colors, allAttchmentList, navigation}) => {
              ...
              <TouchableOpacity
                ...
                <>
                  <View
                    ...
                    <Text style={styles.whitecolor}>{filterData?.length}</Text>
                  </View>
                </>
              </TouchableOpacity>
            ) : null}
...
const SelectParts = ({navigation}) => {
  ...
  useEffect(() => {
    if (isInternetReachable) {
      ...
      dispatch({type: SET_LOADER_FALSE});
      fetchPartLookUpListLocal().then((res) => setPartsList(res));
    }
  }, []);
  ...
  //api function to get part look up list
  const onApplyFilter = () => {
    try {
      dispatch({type: SET_LOADER_TRUE});
      const handleCallback = {
  ...
  const onSuccess = (data) => {
    try {
      data ? setPartNo(`${data?.data}`) : Alert.alert(`Part not found`);
    } catch (error) {
      toggleFilter();
