...
const AddParts = ({navigation}) => {
  ...
  //     value: 'Request Type 2',
  //   },
  //   {
  //     id: 3,
  //     label: 'Request Type 3',
  //     value: 'Request Type 3',
  //   },
  // ];
  ...
  const serialNoData = [
    {
      id: 1,
      serialNo: 'AHC12345678901234567890',
      date: '22/10/2022',
    },
    {
      id: 2,
  ...
  const [serialNoResRawData, setserialNoResRawData] = useState([]);
  ...
  const resetvaluess = () => {
    if (edit) {
      ...
      try {
        // setRequesttype(partsData?.MobPartType);
        const endpoint =
        ...
        const handleCallback = {
          success: (editRequest) => {
            ...
            // setWareHouseId(selectPartsFromList[0]?.WarehouseId)
            setNoteTwo(
  ...
  const toggleSearchModal = () => {
    ...
    // if(selectPartsFromList?.length == 0){
    //   alert('series is empty')
  ...
  const getDataBasedOnType = () => {
    ...
    if (selectedTypeData?.title == 'Normal') {
      ...
      data = {
        ...
        // ETATime: `${restVar}.000Z`,
        RMANo: rma,
        RMATrackingNo: rmaTracking,
        // RMAShippingDate: edit ? null : formatedRmaDates,
        ...
        // LastChangedBy: !edit ? getWorkOrderAppointment?.LastChangedBy :  getWorkOrderAppointment?.LastChangedBy,
        LastChangedBy: null,
        CoreReturnStatusID: null,
        CoreReturnNotes: '',
        partPricing: {
      ...
      //   PartNotes: noteOne,
      //   IsOEM: 1,
      //   IsCoreReturn: checkboxValue == true ? 1 : 0,
      //   OEMName: 'OEM Name API',
      //   OrderNo: order,
      //   ShipVia: shipVia,
      //   ShippingDate: `${shippingDate}T00:00:00`,
      //   TrackingNo: tracking,
      //   ETADate: `${ETADate}T00:00:00`,
      //   ETATime: `${ETADate}T00:00:00`,
      //   RMANo: rma,
      //   RMATrackingNo: rmaTracking,
      //   RMAShippingDate: RMADate,
      //   RMANotes: noteTwo,
      //   WhId: wareHouseData[0]?.WarehouseId,
      //   IsSerialized: selectPartsFromList[0]?.IsSerialized,
      //   loginid: userInfo?.sub,
      //   SerialNo: [],
      //   RequestStatusId: null,//1,
      //   PartOEM: null,//1,
      //   SourceID: 1,//null
      //   CreatedBy: technicianJobInformation?.CreatedBy,
      //   LastChangedBy: null,
      //   CoreReturnStatusID: null,
      //   CoreReturnNotes: "",//null,
      //   partPricing: {
      //     WoPartPriceId: 0,
      //     CompanyId: 0,
      //     WoJobId: 0,
      //     PartRequestId: 0,//null,
      //     PriceTypeId: 270,
      //     Qty: 1,
      //     UnitPrice: selectPartsFromList[0]?.UnitPrice,//0
      //     Description: selectPartsFromList[0]?.Description,
      //     DiscountTypeId: 80,
      //     Discount: 0,
      //     TaskNo: 0,
      //     CreatedBy: technicianJobInformation?.CreatedBy,
      //     CreatedDate: null,
      //     LastChangedBy: 1687,//null,
      //     LastUpdate: null,
      //     CreatedSourceId: null,//1,
      //     UpdatedSourceId: null,
      //     TaxTypeId: selectPartsFromList[0]?.TaxTypeId,
      //     TaxIdGroupId: selectPartsFromList[0]?.TaxIdGroupId,
      //     Tax: null,
  ...
  const handleSave = () => {
    // try {
    //   const data = getPayloadForAPI();
    //   selectPartsFromList?.length > 0
    //     ? addUpdateNormalOEM(data)
    //     : FlashMessageComponent('reject', 'Please select Product');
    // } catch (r) {}
    ...
    <>
                ...
                <View style={styles.mainContainer}>
                  ...
                  <View style={styles.quantityContainer}>
                    ...
                    <TextInput
                      ...
                      value={quantity}
                      // value={edit?selectPartsFromList[0].:quantity}
                      onChangeText={(text) => {
                        setQuantity(text);
                        deCimalCheck(text);
                      }}
                  ...
                  {selectedTypeData?.title == 'OEM' ? (
                    <View style={{ marginTop: normalize(25) }}>
                      ...
                      <Input
                        ...
                        inputContainer={{ marginTop: normalize(8) }}
                  ...
                  {selectedTypeData?.title != 'BOQ' ? (
                    <View style={{ marginTop: normalize(25) }}>
                      <LabelComponents
                        ...
                        required={false}
                      />
                      {/* OEMPartReqTypeId ==1?'Normal':'Quick Response' */}
      ...
      {showModal ? (
        <ConfirmationModal
          ...
          title={strings('confirmation_modal.title')}
          // discription={
          //   'Required quantity is not available in \n van warehouse.\n \nDo you want to create a request for the part against the van warehouse for this job?'
          // }
          discription={
