...
// import { fetchCompleteReason, fetchCustomerDetails, fetchPartRequestList, fetchNotesOnline, fetchProjectDetails, fetchWoJobOnline, fetchTechnicianDetail } from '../../../lib/callBackApi.js';
...
import {
  ...
  fetchCustomFeildsOnline,
} from '../../../lib/callBackApi.js';
import { accessPermission } from '../../../database/MobilePrevi/index.js';
import { GET_CUSTOMER_AND_TECH_DETAILS, GET_TECH_DETAIL_ONLINE } from '../../../lib/api/requestTypes.js';
...
const JobDetail = ({ navigation }) => {
  ...
  const [jobServiceNotesDetail, setjobServiceNotesDetail] = useState(jobDetailsData?.GetWojobServiceNotesDetails)
  const [getTechnicianRemarks, setGetTechnicianRemarks] = useState(jobDetailsData?.GetTechnicianRemarks)
  const [getWojobSplInsDetails, setGetWojobSplInsDetails] = useState(jobDetailsData?.GetWojobSplInsDetails)
  const [getWOJobChecklist, setWOJobChecklist] = useState(jobDetailsData?.GetWOJobChecklist);


  // const getWojobSplInsDetails =
  //   route?.params?.jobDetailsData?.GetWojobSplInsDetails;
  // const getTechnicianRemarks =
  //   route?.params?.jobDetailsData?.GetTechnicianRemarks;
  // const getjobServiceNotesDetail =
  //   route?.params?.jobDetailsData?.jobServiceNotesDetail;
  const { colors } = useColors();
  const dispatch = useDispatch();
  // const { jobId, isCrewMember, workorderid } = route?.params;
  ...
  const [GetAllPartRequestEntity, setGetAllPartRequestEntity] = useState(jobDetailsData?.GetAllPartRequestEntity);
  const [tech, setTech] = useState(false);
  const [orderID, setOrderID] = useState(jobDetailsData?.GetWorkOrderAppointment?.WorkOrderId)
...
console.log('background apiiii',background)
  ...
  const toggleUnresolvedModal = () => {
    // if(technicianJobInformation?.JobStatus == 'On Site'){
    setActualHr(getWorkOrderAppointment?.ActualWorkedHours);
    setActualMins(getWorkOrderAppointment?.ActualWorkedMinutes);
    setActualTravel(getWorkOrderAppointment?.ActualTravelDistance);
  ...
  useEffect(() => {
    if ((workOrderCustomerContactDetails?.Phone1 === null ||
  ...
  const [getWorkOrderAppointment, setgetWorkOrderAppointment] =
    useState(jobDetailsData?.GetWorkOrderAppointment);
  // useState(route?.params?.jobDetailsData?.WorkOrderCustomerContactDetails);

  const [woJobDetails, setWoJobDetails] = useState(jobDetailsData?.woJobDetails);
  // const jobPartList = useSelector(
  //   (state) => state?.jobDetailReducers?.data?.GetPartRequestList?.PartLists,
  // );
  const [jobPartList,setJobPartList] = useState(jobDetailsData?.GetPartRequestList)
  // const getWorkOrderAppointment =
  //   route?.params?.jobDetailsData?.GetWorkOrderAppointment;
  const getProjectDetails = route?.params?.jobDetailsData?.GetProjectDetails;
  const [technicianJobInformation, setTechnicianJobInformation] = useState(jobDetailsData?.TechnicianJobInformation)
  const [contactDetailPopUp, setContactDetailPopUp] = useState(false);
  const [requestPartList, setRequestPartList] = useState(jobDetailsData?.GetPartRequestList);
  ...
  // const fetchJobDetailsFromLocal = () => {
  //   fetchJobDetailsData().then((res) => {
  //     res.map((item) => {
  //       if (JSON.parse(item?.TechnicianJobInformation)?.WoJobId === route?.params?.jobId) {
  //         setTechnicianJobInformation(JSON.parse(item?.TechnicianJobInformation));
  //         setCustomFieldsPanel(JSON.parse(item?.CustomFields));
  //         setCustomertimeZone(JSON.parse(item?.CustomertimeZone));
  //         setGetProjectDetails(JSON.parse(item?.GetPrjobDetails[0]?.WorkOrderCustomerContactDetails));
  //         setRequestPartList(JSON.parse(item?.GetPartRequestList?.PartLists));
  //         setGetWOJobChecklist(JSON.parse(item?.GetWOJobChecklist));
  //         setWOJobDetails(JSON.parse(item?.WOJobDetails));
  //         setGetWorkOrderAppointment(JSON.parse(item?.GetWorkOrderAppointment));
  //         setjobServiceNotesDetail(JSON.parse(item?.GetWojobServiceNotesDetails));
  //         setGetWojobSplInsDetails(JSON.parse(item?.GetWojobSplInsDetails));
  //         setGetTechnicianRemarks(JSON.parse(item?.GetTechnicianRemarks));
  //         setOtpEnabled(JSON.parse(item?.OTPEnable));
  //       }
  //     })
  //   })
  // }
  ...
  //         );
  //         setAttachmentData(jobDetails[0]?.GetAllAttachment)
  //         setRequestPartList(jobDetails[0]?.GetPartRequestList?.PartLists);
  ...
  //         setGetWojobSplInsDetails(jobDetails[0]?.GetWojobSplInsDetails);
  ...
  //         );
  //         // setAttachmentData(jobDetails[0]?.GetAllAttachment);
  //         // setRequestPartList(jobDetails[0]?.GetPartRequestList);
  //         setGetWOJobChecklist(jobDetails[0]?.GetWOJobChecklist);
  //         setWOJobDetails(jobDetails[0]?.WOJobDetails);
  //         setGetWorkOrderAppointment(jobDetails[0]?.GetWorkOrderAppointment);
  ...
  const getOtpReasons = () => {
    try {
      ...
      console.log('catch error otp reason', { error });
    }
  };
  useEffect(() => {
  ...
  const fetchJobDetailRealm = () => {
    const callback = (data) => {
      ...
      // setCompletedJobReasonList(data?.CompletedStatusReasons)
    }
  ...
  const fetchNotesCallback = (id) => {
    if (isInternetReachable) {
      ...
      // let endPoint = `${userInfo?.CompanyId}/28/${getWorkOrderAppointment?.WorkOrderId}/${jobId}/${id}`;
    let endPoint = `${userInfo?.CompanyId}/${userInfo?.sub}/28/${getWorkOrderAppointment?.WorkOrderId}/${jobId}/${id}`;
      // ='416/28/1604475076/19158567/1';
  ...
  const customerDetailCallback = () => {
    const cb = (data) => {
      // dispatch(saveJobDetails({ ...jobDetails, WorkOrderCustomerContactDetails: data }));
    };
  ...
  const statusReasonCallback = () => {
    const cb = (data) => {
      // dispatch(saveJobDetails({ ...jobDetails, CompletedStatusReasons: data }));
    ...
    try {
      if (
        ...
        const handleCallback = {
          ...
          error: (insertOtpError) => console.log("insertOtpError", { insertOtpError }),
        };
        const headers = Header(token);
        api.insertOTP(data, handleCallback, headers);
      } else {
        FlashMessageComponent('reject', strings('Location_Violation.Please_Enter_OTP'));
      }
    } catch (error) {
      console.log('insertOtp catch block', { error });
    }
  };
  ...
  const handleUnResolvedUpdateStatus = (id) => {
    if (reasonId && reasonId.length > 0) {
      ...
      id == 5 ? toggleCompleteModal() : toggleUnresolvedModal();
       FlashMessageComponent('warning', strings('flashmessage.Please_select_a_reason'));
    }
  };
  ...
  const handleUnResolvedUpdateStatusApi = (id) => {
    try {
      ...
      dispatch({ type: SET_LOADER_FALSE });
      console.log('unresolved status error', error);
    }
  };
  ...
  const [attachmentData, setAttachmentData] = useState(jobDetailsData?.GetAllAttachment);
  ...
  const GetAttachments = () => {
    try {
      ...
      // api?.GetAllAttachmentOnline('', handleCallback, headers, endpoint);
    } catch (error) { }
  };
    ...
    try {
      const handleCallback = {
        success: (res) => {
          ...
          // dispatch(saveJobDetails(...Job_Details_Store,GetWOJobChecklist=res));
        },
        error: (attachmentErr) => console.log({ attachmentErr }),
      };
  ...
  // let requestPartList =
  //   route?.params?.jobDetailsData?.GetPartRequestList;
  

  // useEffect(() => {
  //   technicianJobInformation?.WorkOrderId
  //     ? //  getPartRequestList()
  //     (requestPartList = jobPartList)
  //     : null;
  // }, [technicianJobInformation]);

  // const jobPartList = useSelector(
  //   (state) => state?.jobDetailReducers?.data?.GetPartRequestList,
  // );

  // useEffect(() => {
  //   technicianJobInformation?.WorkOrderId
  //     ? //  getPartRequestList()
  //     setRequestPartList(jobPartList)
  //     : null;
  // }, [technicianJobInformation]);
  ...
  //${technicianJobInformation?.JobStatusid}
  ...
  const onResendOTP = () => {
    try {
      ...
      console.log('ResendOtp Catch error', { error });
    }
  };
  ...
  const getOtpDetails = () => {
    try {
      ...
      console.log('otp Detail Catch error', { error });
    }
  };
  ...
  const getMobileOTPTypeID = () => {
    try {
      ...
      console.log('mobile oty type catch error', { error });
    }
  };
  ...
  const validateOtp = () => {
    try {
      ...
      console.log('otp validation catch Error', { error });
    }
  };
  ...
  const getCustomFieldsCallBack = () => {
    ...
    // fetchCustomFeildsOnline('', token, endPoint, cb);
  ...
  return (
    <View style={{ flex: 1 }}>
       ...
       {isConnected=='false' ? 
          ...
          <ScrollView
            ...
            <View style={styles.detailsContainer}>
              <View
                ...
                <Text
                  style={[
                    ...
                    { color: colors?.PRIMARY_BACKGROUND_COLOR,flexWrap:'wrap',flex:1 },
                  ]}>
                    ...
                    onPress={
                      checkIn
                        ...
                        : () => {
                          FlashMessageComponent(
                            'reject',
                            strings('flashmessage.Please_Check_In_to_update_job_status'),
                          );
                        }
                    }>
                    <View>
              ...
              <View style={styles.projectContainer}>
                <View style={styles.nameContainer}>
                  ...
                  {getProjectDetails != null && (
                    <Button
                      ...
                      backgroundColor={Platform.OS === 'android' ? colors?.PRIMARY_BACKGROUND_COLOR : colors?.PRIMARY_BACKGROUND_COLOR}
                      height={'auto'}
                      width={'auto'}
                    />
                  )}
                ...
                <View style={styles.woIdContainer}>
                  {permission?.View && woJobDetails?.map((detail, index) => {
                    return (
                      <JobTaskCards
                        key={`ID-${index}`}
                        cardHeaderStyles={{
                        ...
                        showTaskOption={
                          (userInfo?.sub == detail?.CreatedBy && technicianJobInformation?.IsAccept == 1 &&
                            technicianJobInformation?.SubmittedSource != 2) ? true : false
                        }
                        hideDeleteButton={detail?.CreatedBy === techId ? true : false}
                        handleDelete={() => {
                ...
                <View style={styles.woIdContainer}>
                  <View style={styles.addTaskContainer}>
                    <PlusIcon
                      height={normalize(10)}
                      width={normalize(10)}
                      fill={permission?.Add ? colors?.PRIMARY_BACKGROUND_COLOR : Colors.darkGray}
                    />
                    <Text
                      color={permission?.Add ? colors?.PRIMARY_BACKGROUND_COLOR : Colors.darkGray}
                      style={styles.addTaskText}
                      onPress={technicianJobInformation?.IsAccept == 1 && permission?.Add && technicianJobInformation?.SubmittedSource != 2
                        ? () =>
                          navigation.navigate('AddTask', {
                            data: woJobDetails[0],
                            list: woJobDetails,
                ...
                <CustomFields
                  editable={technicianJobInformation?.IsAccept == 1 &&
                    technicianJobInformation?.SubmittedSource != 2 ? true : false}
            ...
            {/* <Text
            onPress={() =>
              navigation?.navigate('EquipmentAttachment', {jobId: jobId})
            }>
            Go to Equipment Attachment List
          </Text> */}
          </ScrollView>
        ) : (
        ...
        {showMoreOpt ? (
          <AddMoreModal
            ...
            containerStyles={{top: Platform.OS === 'ios' ? normalize(44) : normalize(10)}}
          />
        ) : null}
        ...
        {/* <Text onPress={toggleShowResumeJob}>Resume job</Text> */}
        {showTechRemark ? (
      ...
      {/* <AllCustomeFields
        panelName={'jobList'}
        type={'jobdetials'}
        CustomFields={customeFieldsPanel[0]}
      /> */}
    </View >
  );
};
