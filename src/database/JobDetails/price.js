...
//add new pay amount in Realm
export const _storeLocalAddPayAmountObj = (value) => {
    new Promise((resolve, reject) => {
        Realm.open(databaseOptions)
            .then((realm) => {
                realm.write(() => {
                    let updatingObj = realm.objectForPrimaryKey(JOB_DETAILS, value?.WoJobId);
                    let data = JSON.parse(updatingObj?.GetJobPaymentDetailsEntity)
                    data.push({ ...value, sync: false, AmountId: data.length + 1 })
                    updatingObj.GetJobPaymentDetailsEntity = JSON.stringify(data)
                    resolve();
                });
            })
            .catch((error) => reject(error));
